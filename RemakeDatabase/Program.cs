...
namespace RemakeDatabase
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Insira o nome do banco:");
            ...
            const string connectionString = @"Data Source=.\sqlexpress;Initial Catalog=master;Integrated Security=true";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(sqlCloseConnections, connection))
                {
                    try
                    {
                        ExecuteAndReportProcess(dbName, command, "Removendo conexões existentes com o banco de dados", "Conexões removidas");
                    }
                    catch (Exception e)
                    {
                        ReportErrorAndCloseConnection(e, connection);
